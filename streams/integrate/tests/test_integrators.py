...
class TestIntegrate(object):
    ...
    def test_hernquist_energy(self, name, Integrator):
        ...
        initial_velocity = (np.array([0.0, 30., 0.])*u.km/u.s)\
    ...
    def test_miyamoto_energy(self, name, Integrator):
        ...
        initial_velocity = (np.array([0.0, 200., 0.])*u.km/u.s)\
                                    .to(u.kpc/u.Myr).value # kpc/Myr
    ...
    @pytest.mark.parametrize(("name","Integrator"), [('leapfrog',LeapfrogIntegrator), ])
    def test_isochrone_energy(self, name, Integrator):
        potential = IsochronePotential(units=usys,
                                       m=1E11*u.M_sun,
                                       b=0.26*u.kpc)
        ...
        fig1,fig2 = plot_energies(potential,ts, xs, vs)
        fig1.savefig(os.path.join(plot_path,"isochrone_energy_{0}.png".format(name)))
        fig2.savefig(os.path.join(plot_path,"isochrone_{0}.png".format(name)))
    ...
    def test_log_potential(self, name, Integrator):
        ...
        initial_velocity = (np.array([0.0, 160., 0.])*u.km/u.s)\
                                    .to(u.kpc/u.Myr).value # kpc/Myr
    ...
    @pytest.mark.parametrize(("name","Integrator"), [('leapfrog',LeapfrogIntegrator), ])
    def test_nfw(self, name, Integrator):
        ...
        initial_position = np.array([14.0, 0.0, 0.]) # kpc
        initial_velocity = (np.array([0.0, 220., 0.])*u.km/u.s)\
                                    .to(u.kpc/u.Myr).value # kpc/Myr
        ...
        plot_energies(potential,ts, xs, vs)
        fig1,fig2 = plot_energies(potential,ts, xs, vs)
        fig1.savefig(os.path.join(plot_path,"nfw_energy_{0}.png".format(name)))
        fig2.savefig(os.path.join(plot_path,"nfw_{0}.png".format(name)))