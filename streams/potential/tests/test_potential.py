...
class TestPotentialCreation():
    ...
    def test_point_mass_1d(self):
        ...
        params = {"M" : 2E30} # kg
        potential = Potential(coordinate_system=SphericalCoordinates)
    ...
    def test_point_mass_3d(self):
        ...
        potential = PointMassPotential(length_unit=u.au, **params)
    ...
    def test_miyamoto_nagai_2d(self):
        ...
        miya_potential = Potential(coordinate_system=CylindricalCoordinates)
    ...
    def test_composite_galaxy_3d(self):
        ...
        potential = Potential(coordinate_system=CartesianCoordinates)
...
class TestPotentialFail(object):
    
    def test_add_2d_compoent_to_3d_potential(self):
        potential = Potential(coordinate_system=CartesianCoordinates)
        ...
        def log_halo(params): return lambda x,y: params["v_circ"]**2 / 2. * np.log(x**2 + y**2/params["p"]**2 + params["d"]**2)
        def d_log_halo_dx(params): return lambda x,y: params["v_circ"]**2 * x / (x**2 + y**2/params["p"]**2 + params["d"]**2)
        def d_log_halo_dy(params): return lambda x,y: params["v_circ"]**2 * y / (x**2 + y**2/params["p"]**2 + params["d"]**2) / params["p"]**2
        ...
        potential.add_component("disk", miyamoto_nagai(disk_params), derivs=(d_miyamoto_nagai_dx(disk_params), d_miyamoto_nagai_dy(disk_params), d_miyamoto_nagai_dz(disk_params)))
        with pytest.raises(ValueError):
            potential.add_component("halo", log_halo(halo_params), derivs=(d_log_halo_dx(halo_params),d_log_halo_dy(halo_params)))
...
class TestPotentialAdd(object):
    ...
    def test_composite_galaxy(self):
        ...
        halo_potential = LogarithmicPotentialJZSH(v_circ=(181.*u.km/u.s).to(u.kpc/u.Myr).value, p=1., q=0.75, c=12.)
