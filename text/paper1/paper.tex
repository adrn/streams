...
Lyrae stars using $K$-band observations of these stars in Galactic
...
\begin{equation}
  \Phi_{halo} = v^2 \ln(C_1 x^2 + C_2 y^2 + C_3 xy + (z/q_z)^2 + R_c^2).
\end{equation}
...
it's mass and stellar populations, orbit, rate of destruction, and
even original distribution in populations (other refs??). Moreover,
the phase-space distribution of the debris offers a unique probe of
...
$\sim2-10$~kpc. Combined with Gaia tangential velocity measurement
accuracies ($\sim$2~km/s at 30~kpc) less than the intrinsic dispersion
for much of the stream ($\sigma_v\sim ??$), and ground-based radial
velocities with $\sim10$~km/s accuracy, the measured phase-space
...
method to take advantage of this information and use each star as an
individual potential measure.
...
satellite.  These definitions require an estimate of the mass of the
...
hard constraint). As a statistical measure over all stars, we
construct a 6D distribution of the minimum phase-space vectors for
each star over the individual, integrated orbits. For each star, $i$,
...
data.\footnote{www.astro.virginia.edu/~srm4n/Sgr/data.html} We then
...
correct and an incorrect potential. Visually, it is clear that the
