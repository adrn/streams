...
the Milky Way. If
\begin{equation}
  f(\bs{\theta})=\log \det(\mathrm{Cov}( \bs{A}_{ik})).
\end{equation}

[Still working on this section...need to figure out best way to express what I'm trying to say.] Though this is not explicitly a \emph{likelihood}, optimizing over this statistic is a very simple approximation to performing a maximum-likelihood optimization. Assuming the samples in $\bs{A}_{ik}$ are approximately normal-distributed, the variance is closely related to the Shannon entropy \citep{shannon1948}, $H$,  
\begin{equation}
  H \propto \log \sigma .
\end{equation}
