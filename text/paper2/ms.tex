...
We develop a statistically justified method for inferring the gravitational potential
  ...
  based the idea that these stars
  were close in phase space at some point in the past.
Our method is flexible enough to adapt to any form for the Galactic potential.
...
Our model is probabilistic, with a likelihood function and priors on the parameters.
  These parameters include the present-day phase-space position for each star
  and the progenitor, and thus the method can properly account for both finite observational uncertainties 
  and missing data dimensions.
We test our method on synthetic data sets generated from an N-body simulation
of satellite disruption in a multi-component Milky Way including a triaxial dark matter halo. 
The data sets were generated with
  observational uncertainties to mimic current and near-future surveys of various stars.
% Results:
We find that even with a small number (8) of well-measured stream stars,
  we can infer properties of the triaxial potential ???? with accuracies of order ??? percent ??? and
  missing phase-space coordinates of the stars ??? with accuracies of order ????.
...
\begin{enumerate}
	...
	\item \textbf{Form of the potential:} There is large uncertainty in the radial profile, shape, orientation, and graininess of the outer halo and the constancy of these parameters over distance. Properly describing the potential may require complex analytic functions where computing actions is non-trivial or even impossible.
	...
	\item \textbf{Comparing models to data:} Matching generated streams to observed stellar densities is difficult; models that rely on this must account for observational biases and properly model the underlying background of halo stars. 
...
\end{equation}
Figure~\ref{fig:potential} shows equipotential slices in the Galactic X-Z plane (Y=0) and Y-Z plane (X=0) for a few choices of  $q_1$,  $q_z$, and $\phi$ while holding all other parameters fixed, as described in the figure caption. The parameters used for the simulations are the same as used in \citet{law10}: [TODO].
...
For a satellite galaxy orbiting within a static host potential, mass loss is driven by a combination of tidal stripping by the steady tidal field of the parent system and tidal shocking by the rapidly changing tidal field as the progenitor system moves through pericenter \citep[e.g.,][]{choi09}. On a mildly eccentric orbit, the tidal shocking is subdominant to the steady disruption. The interplay between these two processes as a function of orbital properties is outside our scope; we focus here on the slow removal of stars by steady tidal forcing. 
...
% TODO: slight ~1% bias in values due to particular sample of stars?
...
%TODO: remove exp 3 and explain that it is possible but computationally difficult...
...
% In Section~\ref{sec:intro} we identified several issues that have only been partially solved by other stream modeling methods; \rewinder\ addresses each of these.
%\begin{enumerate}
%	\item Observational uncertainties and missing data dimensions are naturally handled in the inferences by including the true 6D positions of stars and the progenitor as parameters in the model.
%	\item The orbits of the stars and progenitor are directly integrated and only require a function that evaluates the acceleration due to the potential at a given position (and time). 
%	\item Incorporating multiple streams into the inference only requires multiplying likelihoods computed for each stream and progenitor.
%	\item Each star adds constraints on the potential parameters and does not require matching a simulated density to the observed density of stars.
...
%\end{enumerate}
% are naturally handled in the inferences 
