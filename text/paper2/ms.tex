...
\end{equation}
Figure~\ref{fig:R_V} shows distributions of $R$ and $V$ --- computed at $t=t_{\rm ub}$ for each particle --- as a function of progenitor mass from the simulations described in Section~\ref{sec:sims}. In these scaled, log coordinates we model distributions in $R$ and $V$ as Gaussians. We find that for masses $\lesssim10^9M_\odot$, the variance ($\sigma^2$ in plot panels) remains approximately constant and the mean stays fixed at 0. For the largest mass simulation ($2.5\times10^9M_\odot$), the perturbation to the total potential due to the progenitor halo becomes significant and thus a large fraction of the stars are disrupted through interactions with the subhalo. For this article, we ignore covariances between $R$ and $V$. The 
