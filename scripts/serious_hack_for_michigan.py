...
def ln_prior_nfw(p):
    log_sat_mass, alpha, v_c, log_r_s = p
    ...
    if log_r_s < 0.7 or v_c > 4.6: # 2 - 100 kpc
        return -np.inf

    return 0.

def ln_likelihood_nfw(p, dt, nsteps, prog_w, star_w, betas):
    log_sat_mass, alpha, v_c, log_r_s = p
...
def ln_posterior_nfw(p, dt, nsteps, prog_w, star_w, betas):
    ln_pri = ln_prior_nfw(p)
    if np.any(~np.isfinite(ln_pri)):
        return -np.inf

    ll = ln_likelihood_nfw(p, dt, nsteps, prog_w, star_w, betas)
...
def main():
    ...
    # parameters
    true_log_sat_mass = np.log(2.5E5)
    nstars = 16
    dt = -0.1
    ...
    # p0[:,2] = np.random.normal(0.2, 0.02, size=nwalkers) # v_c
    # p0[:,3] = np.random.normal(3., 0.2, size=nwalkers) # log_r_s
