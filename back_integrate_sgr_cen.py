...
ts_forward, xs_forward, vs_forward = leapfrog(galaxy_potential.acceleration_at, initial_position, initial_velocity, t1, t2, sgr_cen.data["dt"][0])
...
initial_position = [sgr_cen.data["x"][-1], sgr_cen.data["y"][-1], sgr_cen.data["z"][-1]]
initial_velocity = [sgr_cen.data["vx"][-1], sgr_cen.data["vy"][-1], sgr_cen.data["vz"][-1]]
t1 = max(sgr_cen.data["t"])
t2 = min(sgr_cen.data["t"])

# Integrate the satellite center backward in the potential from the last timestep data
ts_back, xs_back, vs_back = leapfrog(galaxy_potential.acceleration_at, initial_position, initial_velocity, t1, t2, -sgr_cen.data["dt"][0])
...
ax1.plot(sgr_cen.data["t"], np.sqrt(sgr_cen.data["x"]**2+sgr_cen.data["y"]**2+sgr_cen.data["z"]**2), color='k', alpha=0.75, linewidth=2)
...
ax2.plot(np.sqrt(sgr_cen.data["x"]**2+sgr_cen.data["y"]**2+sgr_cen.data["z"]**2), np.sqrt(sgr_cen.data["vx"]**2+sgr_cen.data["vy"]**2+sgr_cen.data["vz"]**2), color='k', alpha=0.75, linewidth=2)
