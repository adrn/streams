...
;     The chip layout is   c1 = lower left 
;                          c2 = lower right
;                          c3 = upper right
;                          c4 = upper left
;
...
;- 
;--------------------------------------------------------------------
Pro ldss_stitch4, strct, indx, imfinal, write_fits=write_fits, hdr=hdr,$
                 outfile = outfile, inroot = inroot, PLOT=plot,gain=gain


  if  N_params() LT 2  then begin 
      print,'Syntax - ' + $
        'ldss_stitch4, inroot, imfinal, /plot, /write_fits'
      return
  endif 


  ; GAINS ARE RELATIVE TO CHIP 1, DETERMINED FROM FLAT FIELDS
    if NOT keyword_set(gain) then gain=[1.0, 0.895, 0.918 , 0.887]
  ...
  ; SET FILE
    ...
    if keyword_set(inroot) then begin
        inroot = inroot
        biasroot = 'Bias/bias_n1'
    endif
  ...
  ; READ EACH CHIP, BIAS SUBTRACT, TRIM
    ...
    for i=1,4 do begin
      ...
      ; MULTIPLY BY GAIN, TRIM IMAGES
        im = im / gain[i-1]
        imtrim = im[0:2031,0:2031]

     ; COMBINE INTO SINGLE FILE
       if (i eq 1) then imfinal[0:2031,0:2031]      = imtrim
       if (i eq 2) then imfinal[2032:4063,0:2031]   = reverse(imtrim,1)
       if (i eq 3) then imfinal[2032:4063,2032:4063]= reverse(reverse(imtrim,1),2)
       if (i eq 4) then imfinal[0:2031,2032:4063]   = reverse(imtrim,2)  
